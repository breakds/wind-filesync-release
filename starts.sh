...
if [ $? -eq 0 ]
then        
 echo "NewProcess Exists!"
else       
 echo "NewProcess NOT Exists!"
fi
...
if [ ! -f  $RecentDir/lib/new.jar ]
then        
 nohup $RecentDir/jdk/linux/jre/bin/FileSyncClient -jar -Xms30m -Xmx300m fileSyncS.jar auto >/dev/null 2>&1 & 
else
 nohup $RecentDir/jdk/linux/jre/bin/FileSyncClient -jar -Xms30m -Xmx300m Wind.FileSync.Client.jar auto service>/dev/null 2>&1 &   
fi