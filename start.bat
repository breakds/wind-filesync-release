...
if exist "%cd%\lib\new.jar" (start %JAVAPATH%\FileSyncClient.exe -jar -Xms30m -Xmx1000m Wind.FileSync.Client.jar %1)
if not exist "%cd%\lib\new.jar" (call %JAVAPATH%\FileSyncClient.exe -jar AutoUpdate.jar copy)
if not exist "%cd%\lib\new.jar" (start %JAVAPATH%\FileSyncClient.exe -jar -Xms30m -Xmx1000m fileSync.jar %1)
exit